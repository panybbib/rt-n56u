	...
	IN ULONG MsgLen)
{
	...
	*/
	if ((pInfo->pAddr[0] & 0x01) == 0x01) {
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MlmeAddBAReqSanity fail - multicast address not support BA\n"));
		return FALSE;
	}
	...
	IN ULONG MsgLen)
{
	...
	if ((MsgLen != sizeof(MLME_ADDBA_RESP_STRUCT))) {
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: message lenght not correct\n", __func__));
		return FALSE;
	}

	if (!VALID_UCAST_ENTRY_WCID(pAd, pInfo->wcid)) {
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: The peer Mac is not associated yet\n", __func__));
		return FALSE;
	}

	if ((pInfo->tid & 0xf0)) {
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: The peer TID is incorrect\n", __func__));
		return FALSE;
	}
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		*/
		...
		case IE_VENDOR_SPECIFIC:
			...
			if (SubType == SUBTYPE_PROBE_RSP) {
				MTWF_LOG(DBG_CAT_BF, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): pEid->Octet[0-2] = %x, %x, %x\n"
		...
		case IE_EXT_SUPP_RATES:
			parse_support_ext_rate_ie(rate, pEid);
		...
		case IE_MULTIPLE_BSSID:
			parse_mbssid_subelement(pAd, wdev, (UINT8 *)pEid,
			ie_list);
			break;
#endif
...
#ifdef WIDI_SUPPORT
			MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s Sanity Failed\n", __func__));
#endif /* WIDI_SUPPORT */
...
#endif /* WSC_INCLUDED */
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("STA: %p\n", pStaCfg));
#endif /* CONFIG_STA_SUPPORT */
		return TRUE;
	} else {
