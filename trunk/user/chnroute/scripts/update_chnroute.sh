...
if [ -z "$CHNROUTE_URL" ]; then
	curl -k -s --connect-timeout 10 --retry 3 http://ftp.apnic.net/apnic/stats/apnic/delegated-apnic-latest | \
		awk -F\| '/CN\|ipv4/ { printf("%s/%d\n", $4, 32-log($5)/log(2)) }' > /tmp/chinadns_chnroute.txt
else
	curl -k -s --connect-timeout 10 --retry 3 -o /tmp/chinadns_chnroute.txt "$CHNROUTE_URL"
fi
